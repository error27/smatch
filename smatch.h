...
enum hook_type {
	...
	LOGIC_HOOK,
	CONDITION_HOOK,
	PRE_LOOP_CONDITION_TWO,
	COMPARE_HOOK,
	FLIPPED_COMPARE_HOOK,
	PRELOOP_HOOK,
	POSTLOOP_HOOK,
...
typedef void (stree_func)(struct stree *stree);
...
void call_stree_hooks(struct stree_func_list *list);
...
extern bool SSA_POINTER_DISABLED;
...
bool get_key_from_var_sym(const char *name, struct symbol *sym,
			  struct expression *ret_expr, const char **key);
...
extern unsigned long __parse_id_cur;
...
struct sm_state *alloc_sm_state_perm(int owner, const char *name, struct symbol *sym,
				     struct smatch_state *state);
